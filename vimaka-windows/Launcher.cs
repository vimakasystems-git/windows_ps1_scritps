...
class Launcher {
 ...
 [STAThread] static int Main(string[] args){try{
  ...
  using(var payload=Assembly.GetExecutingAssembly().GetManifestResourceStream("payload.zip")){
   if(payload!=null){
    ...
    using(var key=Registry.CurrentUser.CreateSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\VimakaWindowsCare")){
     key.SetValue("DisplayName","Vimaka Windows Care");key.SetValue("DisplayVersion","0.1.9");key.SetValue("Publisher","Vimaka Sistemas Inteligentes");
  ...
  if(Replaced)File.WriteAllText(Path.Combine(Root,"last-install.txt"),"0.1.9 OK "+DateTime.UtcNow.ToString("o")+"\nPrevious version: "+Backup);
  return 0;
