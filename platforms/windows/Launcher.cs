 ...
 [STAThread] static int Main(string[] args){try{
  ...
  using(var payload=Assembly.GetExecutingAssembly().GetManifestResourceStream("payload.zip")){
   if(payload!=null){
    ...
    string plan=Text("Instalar/atualizar Vimaka Workstation Care e Node.js incluido (executa o componente local). A versao anterior sera substituida; diagnosticos e historico serao preservados. Nao exige administrador. Nenhum reparo sera executado.","Install/update Vimaka Workstation Care and bundled Node.js (runs the local component). The previous version will be replaced; diagnostics and history will be preserved. No administrator required. No repairs will run.","Instalar/actualizar Vimaka Workstation Care y Node.js incluido (ejecuta el componente local). Se sustituira la version anterior y se conservaran los datos. No requiere administrador. No se ejecutaran reparaciones.");
    ...
    if((!acceptInstall||needsEdge)&&MessageBox.Show(plan,"Vimaka Workstation Care",MessageBoxButtons.OKCancel,MessageBoxIcon.Information)!=DialogResult.OK)return 0;
    ...
    string exe=Path.Combine(AppDir,"VimakaWorkstationCare.exe");
    string desktop=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory),"Vimaka Workstation Care.lnk");
    string menu=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs),"Vimaka Workstation Care.lnk");
    ...
    foreach(var folder in new[]{Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory),Environment.GetFolderPath(Environment.SpecialFolder.Programs)}){
      string oldLink=Path.Combine(folder,"Vimaka Windows Care.lnk");if(File.Exists(oldLink))File.Delete(oldLink);
    }
    ...
    using(var key=Registry.CurrentUser.CreateSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\VimakaWindowsCare")){
     key.SetValue("DisplayName","Vimaka Workstation Care");key.SetValue("DisplayVersion","@@VERSION@@");key.SetValue("Publisher","Vimaka Sistemas Inteligentes");
  ...
  if(!Online()){
    ...
    if(!File.Exists(node)||!File.Exists(server))throw new Exception(Text("Instalacao nao encontrada. Execute VimakaWorkstationCare-Setup.exe.","Installation not found. Run VimakaWorkstationCare-Setup.exe.","Instalacion no encontrada. Ejecute VimakaWorkstationCare-Setup.exe."));
  ...
  if(Replaced)File.WriteAllText(Path.Combine(Root,"last-install.txt"),"@@VERSION@@ OK "+DateTime.UtcNow.ToString("o")+"\nPrevious version: "+Backup);
  return 0;
 }catch(Exception e){if(Backup!=null){try{if(Directory.Exists(AppDir))InstallSupport.Move(AppDir,Path.Combine(Root,"failed-"+Guid.NewGuid().ToString("N")));InstallSupport.Move(Backup,AppDir);}catch(Exception rollback){e=new Exception(e.Message+"\nRollback: "+rollback.Message);}}File.WriteAllText(Path.Combine(Path.GetTempPath(),"Vimaka-install-error.txt"),e.ToString());MessageBox.Show(e.Message,"Vimaka Workstation Care",MessageBoxButtons.OK,MessageBoxIcon.Warning);return 1;}}
}
