...
export function setupDashboard({api,refresh,confirmAction,notice,showPage}){
 ...
 $('export-machine').onclick=()=>{
  ...
  const html=`<!doctype html><html lang="${locale()}"><meta charset="utf-8"><title>${escape(title)}</title><style>body{font:16px/1.6 system-ui;max-width:1100px;margin:32px auto;padding:24px;color:#0f1729}pre{white-space:pre-wrap;overflow-wrap:anywhere}td,th{padding:10px;border-bottom:1px solid #ddd;text-align:left}table{width:100%}</style><h1>${escape(title)}</h1><p>Vimaka Workstation Care · ${escape(new Date().toLocaleString(locale()))}</p><h2>${escape(t('Configuração da máquina'))}</h2>${$('hardware-summary').innerHTML}<h2>${escape(t('Antes e agora'))}</h2>${$('comparison').innerHTML}<h2>${escape(t('Benchmark local'))}</h2>${$('benchmark-results').innerHTML}<p>${escape(t('Microteste local; não representa desempenho geral nem é comparável ao Geekbench.'))}</p><h2>${escape(t('Etapas da execução'))}</h2>${$('execution').innerHTML}${$('diagnosis-result').innerHTML}${$('performance-result').innerHTML}<h2>${escape(t('Configuração completa'))}</h2><pre>${escape(JSON.stringify(currentState.comparison.current,null,2))}</pre><p>${escape(t('Relatório local. Revise antes de compartilhar.'))}</p></html>`;
